...
function route() {
  ...
  else if (hash === 'exam') renderExamSetup(app);
  else if (hash === 'navflow') renderNavFlow(app);
  ...
  const navMap = {home:'nav-home', calculators:'nav-calc', reference:'nav-reference', ref:'nav-reference', progress:'nav-progress', exam:'nav-exam', navflow:'nav-navflow'};
...
function renderExamSetup(app) {
  ...
  app.innerHTML = `
    <div class="section wrap">
      <h2>Exam mode</h2>
      <p class="sub">Timed test, shuffled questions, pass at 70%. Pick which modules to include.</p>
      <div class="exam-setup">
        <label class="exam-check"><input type="checkbox" id="exam-all" checked> <strong>All modules (${coreModules.reduce((s,m)=>s+m.mcqs.length,0)} questions)</strong></label>
        <div id="exam-module-list" style="display:none">
          ${coreModules.map(m => `
            <label class="exam-check"><input type="checkbox" class="exam-mod-cb" value="${m.id}"> Module ${m.id} &mdash; ${m.title} (${m.mcqs.length})</label>
          `).join('')}
        </div>
        <div class="field" style="max-width:220px;margin-top:16px">
          <label>Time limit (minutes)</label>
          <input id="exam-minutes" type="number" value="20">
        </div>
        <button class="btn" id="exam-start-btn" style="margin-top:18px">Start exam &rarr;</button>
      </div>
    </div>
  `;
  document.getElementById('exam-all').addEventListener('change', (e) => {
    document.getElementById('exam-module-list').style.display = e.target.checked ? 'none' : 'block';
  });
  document.getElementById('exam-start-btn').addEventListener('click', () => {
    const all = document.getElementById('exam-all').checked;
    let pool = [];
    if (all) {
      coreModules.forEach(m => m.mcqs.forEach(q => pool.push({...q, moduleTitle: m.title})));
    } else {
      const checked = Array.from(document.querySelectorAll('.exam-mod-cb:checked')).map(cb => parseInt(cb.value,10));
      coreModules.filter(m => checked.includes(m.id)).forEach(m => m.mcqs.forEach(q => pool.push({...q, moduleTitle: m.title})));
    }
    if (!pool.length) { alert('Pick at least one module.'); return; }
    pool = pool.sort(() => Math.random() - 0.5);
    const minutes = parseFloat(document.getElementById('exam-minutes').value) || 20;
    runExam(app, pool, minutes);
  });
}

let examTimerInterval = null;

function runExam(app, questions, minutes) {
  ...
  function renderQuestion() {
    ...
    app.innerHTML = `
      <div class="section wrap exam-runner">
        <div class="exam-header">
          <div class="exam-progress">Question ${current+1} / ${questions.length}</div>
          <div class="exam-timer" id="exam-timer">--:--</div>
        </div>
        <div class="quiz-panel">
          <div class="exam-mod-tag">${q.moduleTitle}</div>
          <div class="q-text" style="font-size:16px;margin-top:10px">${q.q}</div>
          ${q.options.map((opt,oi) => `<button class="opt exam-opt" data-oi="${oi}">${String.fromCharCode(65+oi)}) ${opt}</button>`).join('')}
        </div>
      </div>
    `;
    document.querySelectorAll('.exam-opt').forEach(btn => {
      btn.addEventListener('click', () => {
        const oi = parseInt(btn.dataset.oi, 10);
        answers.push(oi === q.answer);
  ...
  function updateTimer() {
    ...
    if (secondsLeft <= 0) { clearInterval(examTimerInterval); finishExam(); return; }
    secondsLeft--;
  }
    ...
    app.innerHTML = `
      <div class="section wrap">
        <div class="exam-result-card">
          <div class="exam-result-pct ${passed?'pass':'fail'}">${pct}%</div>
          <h2>${passed ? 'Passed' : 'Not quite'}</h2>
          <p class="sub">${score} / ${questions.length} correct &middot; pass mark is 70%</p>
          ${passed ? `
            <div class="field" style="max-width:280px;margin:20px 0"><label>Your name for the certificate</label><input id="cert-name" type="text" placeholder="Full name"></div>
            <button class="btn" id="gen-cert-btn">Generate certificate &rarr;</button>
          ` : `<a class="btn ghost" href="#exam">Try again</a>`}
          <div style="margin-top:16px"><a href="#home">&larr; Back to modules</a></div>
        </div>
      </div>
    `;
    if (passed) {
      document.getElementById('gen-cert-btn').addEventListener('click', () => {
        ...
        renderCertificate(app, name, pct, questions.length);
      });
    }
  }
...
function renderCertificate(app, name, pct, totalQ) {
  const date = new Date().toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' });
  app.innerHTML = `
    <div class="section wrap">
      <div class="certificate">
        <div class="cert-mark">PE</div>
        <div class="cert-label">Certificate of Completion</div>
        <div class="cert-name">${name}</div>
        <div class="cert-body">has successfully completed the PE Fund Accounting exam<br>with a score of <strong>${pct}%</strong> across ${totalQ} questions</div>
        <div class="cert-date">${date}</div>
        <div class="cert-sign">The PE Fund Accountant &mdash; self-study companion</div>
      </div>
      <div style="text-align:center;margin-top:20px">
        <button class="btn" onclick="window.print()">Print / Save as PDF</button>
        <a class="btn ghost" href="#home" style="margin-left:10px">Back to modules</a>
      </div>
    </div>
  `;
}

// ---- interactive NAV workflow diagram ----
const NAV_STAGES = [
  { title: 'Transaction occurs', detail: 'A capital call, investment, income receipt, or expense payment happens.' },
  { title: 'Recorded in the books', detail: 'The fund accountant records the journal entry (Module 4).' },
  { title: 'Matched against custodian', detail: 'Internal records are reconciled against the custodian\'s independent record (Module 11.2).' },
  { title: 'Valuations updated', detail: 'Portfolio company fair values are refreshed each quarter (Module 5).' },
  { title: 'NAV calculated', detail: 'Assets minus liabilities, at fair value, produces the fund-level NAV (Module 6.1).' },
  { title: 'Reviewed and tied out', detail: 'Every number is checked against supporting detail before finalizing (Module 6.5).' },
  { title: 'Reported to LPs and GP', detail: 'Capital account statements and financials are issued (Module 13.2).' },
];

function renderNavFlow(app) {
  app.innerHTML = `
    <div class="section wrap">
      <h2>NAV process workflow</h2>
      <p class="sub">Click any stage to see what happens there. Same flow as Module 11.1, end to end.</p>
      <div class="navflow">
        ${NAV_STAGES.map((s, i) => `
          <div class="navflow-stage" data-i="${i}">
            <div class="navflow-num">${i+1}</div>
            <div class="navflow-title">${s.title}</div>
          </div>
          ${i < NAV_STAGES.length-1 ? '<div class="navflow-arrow">&rarr;</div>' : ''}
        `).join('')}
      </div>
      <div id="navflow-detail" class="navflow-detail">Click a stage above to see the detail here.</div>
    </div>
  `;
  document.querySelectorAll('.navflow-stage').forEach(el => {
    el.addEventListener('click', () => {
      document.querySelectorAll('.navflow-stage').forEach(s => s.classList.remove('active'));
      el.classList.add('active');
      const i = parseInt(el.dataset.i, 10);
      document.getElementById('navflow-detail').innerHTML =
        `<strong>${i+1}. ${NAV_STAGES[i].title}</strong><p>${NAV_STAGES[i].detail}</p>`;
    });
  });
}
